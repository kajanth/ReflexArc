...
class NSAOrchestrator:
    ...
    def __init__(self, db_path: str = "memory/long_term_memory.db",
        ...
        # ⚡ Bolt: Cache available skills to prevent sync os.listdir bottleneck
    ...
    async def process_spike(self, sense_type: str, description: str) -> Optional[str]:
        ...
        # ⚡ Bolt: Cache available skills to prevent sync os.listdir bottleneck
        try:
            skills_dir_mtime = os.path.getmtime('skills')
        except OSError:
            skills_dir_mtime = 0.0

        if self._skills_cache is None or skills_dir_mtime != self._skills_mtime:
            self._skills_cache = [f.replace(".py", "") for f in os.listdir('skills') if f.endswith('.py')]
            self._skills_mtime = skills_dir_mtime
