...
class NetworkProbeSensor:
    ...
    async def monitor(self):
        ...
        # ⚡ Bolt: Run probes concurrently to prevent event loop blocking
        # and reduce total latency from O(N) to O(1)
        tasks = [asyncio.to_thread(self._probe_endpoint, host, port) for host, port in self.endpoints]
